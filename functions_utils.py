...
def beam_envelope_supergaussian(cords, N, amp=1, alpha=1, sigma=1, super_g_x_zero=1, super_g_y_zero=1):
	...
	return envelope
def beam_envelope_sinc(x, y, width, power):
	alpha	= 1 / width
	sinc_x	= np.sin(x * alpha) / (x * alpha)
	sinc_y	= np.sin(y * alpha) / (y * alpha)
	return (sinc_x * sinc_y)**power
...
# applying ssd and finding far field intensity distribution
	...
	if do_isi:
		int_ff	= util_perform_isi(int_beam_env, x, y, area_nf, area_ff, beam_power,
							...
							dpp=dpp, do_dpp=True, echelon_block_width=echelon_block_width, sf=sf)
...
def util_for_plot_moving_speckles_ssd(int_beam_env, int_ff_env, near_field, timesteps, time, time_resolution, pcm, point_artist, fig, ax, area_ff, beam_power, nf, ff, coords, do_cumulative=False, show_com=False):
	...
	for idx, near_field_t in tqdm(enumerate(apply_smoothing_by_spectral_dispersion(
		...
		# Compute instantaneous far-field intensity
		int_ff_inst = util_compute_fft(near_field_t)
		int_ff_inst = util_scale_int_env(int_ff_inst, area=area_ff, beam_power=beam_power)

		# Accumulate over time (same as perform_fft_normalize)
		if do_cumulative:
			int_ff_cumulative += int_ff_inst
			int_ff_display = int_ff_cumulative / (idx + 1)
		else:
			int_ff_display = int_ff_inst

		# Normalize to relative intensity (for consistent visualization)
		_, _, _, _, _, int_ff_display = util_find_relative_intensity(
			nf_x, ff_x, nf_y, ff_y, int_ff_env, int_ff_display
		)
...
def util_for_plot_moving_speckles_isi(int_beam_env, x, y, area_nf, area_ff, beam_power,
	...
	for idx, timestep in tqdm(enumerate(timesteps), desc="Applying isi"):	
		...
		if do_cumulative:
			...
			int_ff_display = util_scale_int_env(int_ff_component, area_ff, beam_power)
